...
To kill it, we simply verified the version property when creating an XML file, in the `ProjectSerializerTest.java` file.
...
Thus, in order to kill it, we added asserts for the missing parameters, in the `ProjectSerializerTest.java` file.
...
Thus, in order to kill them, we added the necessary parameters to different projects, in the `ProjectSerializerTest.java` file.
...
To kill these mutants, we used a *spy* object and verified that these methods were being called with the correct parameters, in the `ProjectTableModelTest.java` file.
